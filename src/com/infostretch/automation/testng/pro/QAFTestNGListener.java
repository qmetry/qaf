...
import com.infostretch.automation.core.HtmlCheckpointResultFormatter;
...
import com.infostretch.automation.step.client.text.BDDTestFactory;
import com.infostretch.automation.testng.MethodPriorityComparator;
...
public class QAFTestNGListener {
	...
	public void transform(IDataProviderAnnotation arg0, Method method) {
		boolean parallel = (getBundle().getInt(method.getName() + ".parallel", 0) == 1)
				|| (getBundle().getInt("global.datadriven.parallel", 0) == 1);
	...
	public void transform(IConfigurationAnnotation iConfigurationAnnotation, Class arg1, Constructor arg2,
			Method arg3) {
	}
	...
	public void beforeInvocation(IInvokedMethod method, ITestResult tr, ITestContext context) {
		...
		ConfigurationManager.getBundle().setProperty("tng.context", context);

		ConfigurationManager.getBundle().setProperty("current.testcase.name", tr.getName());
		ConfigurationManager.getBundle().setProperty("current.testcase.desc", tr.getMethod().getDescription());
	}
	...
	public void afterInvocation(final IInvokedMethod method, final ITestResult tr, final ITestContext context) {
		logger.debug("afterInvocation: " + method.getTestMethod().getMethodName() + " - "
				+ method.getTestMethod().getConstructorOrMethod().getDeclaringClass().getPackage().getName()
	...
	protected void processResult(ITestResult tr, ITestContext context) {
		...
		if ((stb.getVerificationErrors() > 0) && (tr.getStatus() == ITestResult.SUCCESS)) {
			setFailure(tr, context);
		}

		if (tr.getStatus() == ITestResult.FAILURE) {
			String failiremsg = getFailureMessage(tr.getThrowable());
			CheckpointResultBean lastFailedChkPoint = getLastFailedCheckpointResultBean(stb.getCheckPointResults());
			...
			if (null != lastFailedChkPoint) {
				...
				if (StringUtil.isBlank(lastFailedChkPoint.getScreenshot())) {
					...
					CheckpointResultBean lastFailedSubChkPoint = getLastFailedCheckpointResultBean(
							lastFailedChkPoint.getSubCheckPoints());

					if (lastFailedSubChkPoint != null && StringUtil.isNotBlank(lastFailedSubChkPoint.getScreenshot())) {
		...
		if (getBundle().getInt("testng.version", 6) > 5) {
			...
			if ((null != context.getFailedTests())) {
				if (((null != context.getFailedTests().getResults(tr.getMethod())))
						&& (context.getFailedTests().getResults(tr.getMethod()).size() > 1)) {
	...
	private void generateTestFailedXML(ISuite suite) throws IOException {
		...
		for (Entry<String, ISuiteResult> suiteResult : suiteResults.entrySet()) {
			IResultMap failedTests = suiteResult.getValue().getTestContext().getFailedTests();
			IResultMap skippedTests = suiteResult.getValue().getTestContext().getSkippedTests();
			...
			if (!failedOrSkipppedMethods.isEmpty()
					&& Scenario.class.isAssignableFrom(failedOrSkipppedMethods.iterator().next().getRealClass())) {
				if (xmlSuite == null) {
				...
				test.setParameters(suiteResult.getValue().getTestContext().getCurrentXmlTest().getTestParameters());
					...
					if (Scenario.class.isAssignableFrom(testMethod.getRealClass())) {
						...
						if (testMethod.getInstance() instanceof Scenario) {
							...
							scriptDataPart = scriptDataPart + String.format(
									"testngMethod.getMethodName().equalsIgnoreCase(\"%s\")", scenario.getTestName());
						}
					}
				}
		...
		if (xmlSuite != null) {
			File file = new File(suite.getOutputDirectory().replace(suite.getName(), "") + "testng-failed-qas.xml");
