...
/**
 * com.infostretch.automation.step.JavaStepReporter.java
 * 
 * @author chirag
 */
@Aspect
public class JavaStepReporter {
	...
	public Object javaTestStep(ProceedingJoinPoint jp, JoinPoint.StaticPart thisJoinPointStaticPart) throws Throwable {
		...
		if (ConfigurationManager.getBundle().getBoolean("method.recording.mode", false)) {
			...
			if (null != testStep) {
				...
				testStep = (JavaStep) TestBaseProvider.instance().get().getContext().getProperty("current.teststep");
