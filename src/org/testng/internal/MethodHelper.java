...
public class MethodHelper {
	...
	private static final Map<ITestNGMethod, String> CANONICAL_NAME_CACHE = new ConcurrentHashMap<ITestNGMethod, String>();
	 ...
	 */
	public static ITestNGMethod[] collectAndOrderMethods(List<ITestNGMethod> methods, boolean forTests, RunInfo runInfo,
			IAnnotationFinder finder, boolean unique, List<ITestNGMethod> outExcludedMethods) {
		List<ITestNGMethod> includedMethods = Lists.newArrayList();
		MethodGroupsHelper.collectMethodsByGroup(methods.toArray(new ITestNGMethod[methods.size()]), forTests,
		...
		for (String fullyQualifiedRegexp : m.getMethodsDependedUpon()) {
			...
			if (!foundAtLeastAMethod) {
				...
				if (maybeReferringTo != null) {
					throw new TestNGException(canonicalMethodName + "() is depending on method " + maybeReferringTo
	...
	protected static ExpectedExceptionsHolder findExpectedExceptions(IAnnotationFinder finder, Method method) {
		...
		IExpectedExceptionsAnnotation expectedExceptions = finder.findAnnotation(method,
	...
	private static Graph<ITestNGMethod> topologicalSort(ITestNGMethod[] methods, List<ITestNGMethod> sequentialList,
		...
		for (ITestNGMethod m : methods) {
			...
			if (methodsDependedUpon.length > 0) {
				ITestNGMethod[] methodsNamed = MethodHelper.findDependedUponMethods(m, methods);
			...
			if (groupsDependedUpon.length > 0) {
				for (String group : groupsDependedUpon) {
					ITestNGMethod[] methodsThatBelongToGroup = MethodGroupsHelper.findMethodsThatBelongToGroup(m,
							methods, group);
	...
	protected static String calculateMethodCanonicalName(Method m) {

		return calculateMethodPackageName(m) + "." + m.getName();
	}

	private static String calculateMethodPackageName(Method m) {
	...
	protected static String calculateMethodCanonicalName(ITestNGMethod m) {
		...
		result = calculateMethodPackageName(m.getConstructorOrMethod().getMethod()) + "." + m.getMethodName();
		CANONICAL_NAME_CACHE.put(m, result);
		return result;
	}

	private static List<ITestNGMethod> sortMethods(boolean forTests, List<ITestNGMethod> allMethods,
		...
		if (!forTests && (allMethodsArray.length > 0)) {
			...
			boolean before = m.isBeforeClassConfiguration() || m.isBeforeMethodConfiguration()
					|| m.isBeforeSuiteConfiguration() || m.isBeforeTestConfiguration();
